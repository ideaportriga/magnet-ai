/**
 * CRUD tests for Model Providers via the UI.
 */

describe('CRUD — Model Providers', () => {
  const testPrefix = 'e2e-test-'

  it('creates a new model provider', () => {
    cy.viewport(1920, 1080)
    cy.visit('#/model-providers')
    cy.wait(3000)
    cy.dismissErrors()
    cy.g('new-btn').click()
  ...
  it('finds the created provider in the list', () => {
    cy.viewport(1920, 1080)
    cy.visit('#/model-providers')
    cy.wait(3000)
    cy.dismissErrors()
  ...
  it('opens provider detail page', () => {
    ...
    cy.url().should('match', /model-providers\/[a-f0-9-]+/)
  })

  it('deletes the created provider', () => {
    cy.viewport(1920, 1080)
    cy.visit('#/model-providers')
    cy.wait(3000)
    cy.dismissErrors()
