...
// Replaces: knowledgeGraph Vuex module for retrieval change tracking and callbacks
export const useKnowledgeGraphPageStore = defineStore('knowledgeGraphPage', () => {
  const isRetrievalChanged = ref(false)
  const pendingSaveCallback = ref<(() => Promise<void>) | null>(null)
  const pendingRevertCallback = ref<(() => void) | null>(null)

  function setRetrievalChanged(changed: boolean) {
    isRetrievalChanged.value = changed
  }

  function setSaveCallback(callback: (() => Promise<void>) | null) {
    pendingSaveCallback.value = callback
  }

  function setRevertCallback(callback: (() => void) | null) {
