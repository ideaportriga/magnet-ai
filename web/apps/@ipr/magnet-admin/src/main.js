...
// references hashed chunks (e.g. `app-Bxy7…css`) that no longer exist. Vite's
// dynamic `import()` rejects with a preload error, Vue Router's lazy route
// load throws, and the UI freezes mid-navigation. Reload to pick up the fresh
// manifest; the 30s window prevents a tight reload loop if the deploy itself
// is broken.
