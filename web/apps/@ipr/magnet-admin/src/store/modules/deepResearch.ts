...
interface DeepResearchConfig {
  ...
  webhook_tool_server?: string | null
  webhook_tool_name?: string | null
...
const actions = {
  ...
  async createRun({ commit, dispatch, rootGetters }: any, { config, input, client_id }: { config?: DeepResearchConfig; input: Record<string, any>; client_id?: string }) {
    commit('setLoading', true)
    try {
      const response = await fetchData({
        method: 'POST',
        ...
        credentials: rootGetters.config?.credentials,
        body: JSON.stringify({ config, input, client_id }),
        headers: {
          'Content-Type': 'application/json',
        },
      })
