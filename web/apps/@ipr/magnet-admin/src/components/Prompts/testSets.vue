...
export default {
  ...
  computed: {
    ...
    sampleTestSet: {
      get() {
        return this.$store.getters.promptTemplateVariant?.sample_test_set || ''
      },
      set(value) {
        this.$store.commit('updateNestedPromptTemplateProperty', { path: 'sample_test_set', value })
      },
    },
  ...
  methods: {
    selectRecord(row) {
      this.$store.commit('setPromptTemplateTestSetItem', row)
    },
  },
}
</script>
