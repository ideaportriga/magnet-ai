<template>
  <km-popup-confirm :visible="showNewDialog" :title="m.dialog_newNoteTakerSettings()" :confirm-button-label="m.common_save()" :cancel-button-label="m.common_cancel()" :notification="m.hint_configurePromptsAfterSaving()" @confirm="createConfig" @cancel="emit(&quot;cancel&quot;)">
    <div class="stack" data-gap="lg">
      <div class="flex-1">
        <div class="km-field text-secondary-text pb-xs pl-sm">{{ m.common_name() }}</div>
        <div class="full-width">
          <km-input v-if="showNewDialog" ref="nameRef" v-model="name" height="30px" :placeholder="m.dialog_newNoteTakerSettings()" :rules="[required()]" />
        </div>
      </div>
      <div class="flex-1">
