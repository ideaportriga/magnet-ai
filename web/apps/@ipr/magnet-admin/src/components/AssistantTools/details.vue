<template>
  ...
  <layouts-details-layout v-if="!loading" :name="name" :description="description" :system-name="system_name" :system-name-rules="[validSystemName()]" :created-at="activeAssistantToolDB?.created_at" :updated-at="activeAssistantToolDB?.updated_at" :created-by="activeAssistantToolDB?.created_by" :updated-by="activeAssistantToolDB?.updated_by" show-record-info @update:name="name = $event" @update:description="description = $event" @update:system-name="system_name = $event">
    <template #header-actions>
      <km-btn v-if="isDirty" :label="m.common_revert()" icon="undo" icon-size="16px" flat @click="revert()" />
      <km-btn :label="m.common_save()" flat icon="save" icon-size="16px" :loading="saving" :disable="saving || !isDirty" @click="handleSave" />
      <ds-dropdown-menu-root>
        ...
        <ds-dropdown-menu-content side="bottom" align="end" :side-offset="4">
          <ds-dropdown-menu-item @select="showNewDialog = true">{{ m.common_clone() }}</ds-dropdown-menu-item>
