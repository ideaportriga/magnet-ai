...
const save = async () => {
  ...
  try {
    ...
    q.notify({
      position: 'top',
      message: 'Knowledge Source Provider has been saved.',
      color: 'positive',
      textColor: 'black',
      timeout: 1000,
    })
    ...
    q.notify({
      position: 'top',
      message: 'Failed to save Knowledge Source Provider.',
      color: 'negative',
      textColor: 'white',
      timeout: 2000,
    })
...
const deleteServer = async () => {
  ...
  try {
    ...
    const errorMessage = error?.message || 'Failed to delete Knowledge Source Provider.'
    q.notify({
      position: 'top',
