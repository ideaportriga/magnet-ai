...
q-dialog(:model-value='showNewDialog', @cancel='$emit("cancel")')
  q-card.card-style(style='min-width: 800px')
    q-card-section.card-section-style
      .row
        .col
          .km-heading-7 {{ m.agents_newTopic() }}
        .col-auto
          q-btn(icon='close', flat, dense, @click='$emit("cancel")')
    q-card-section.card-section-style.q-mb-md
      .row.items-center.justify-center
        km-stepper.full-width(
          :steps='[ { step: 0, description: m.agents_topicSettingsStep(), icon: "pen" }, { step: 1, description: m.agents_addActionsStep(), icon: "circle" }, ]',
          :stepper='stepper'
        )
      .column.full-width(v-if='stepper === 0')
        km-notification-text.q-mb-md(:notification='m.hint_topicNameDescription()')
        .km-field.text-secondary-text.q-pl-8.q-mb-md {{ m.common_name() }}
        km-input.q-mb-md(height='30px', v-model='name', ref='nameRef')
        .km-field.text-secondary-text.q-pl-8 {{ m.agents_descriptionForLlmShort() }}
        km-input.q-mb-sm(type='textarea', rows='4', v-model='topic.description', ref='descriptionRef')
        km-notification-text.q-mb-md(:notification='m.hint_topicLlmInstructions()')
        ...
        km-input.q-mb-md(height='30px', v-model='system_name', ref='system_nameRef')
          .km-description.text-secondary-text.q-pb-4 {{ m.hint_systemNameAutoGenerated() }}
      ...
      .row.q-mt-lg
        template(v-if='stepper === 0')
          ...
          .col
          .col-auto
            km-btn(:label='m.common_next()', @click='proceed', :disable='!readyForNext')
        ...
        template(v-else)
          ...
          .col
          .col-auto.q-mr-sm
            km-btn(v-if='stepper != 0', flat, :label='m.common_previous()', color='primary', @click='stepper--')
          .col-auto
