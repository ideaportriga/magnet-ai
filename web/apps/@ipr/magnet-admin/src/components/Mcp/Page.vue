      ...
      <template v-else>
        <div class="cluster" data-justify="center">
          <div class="flex-none p-xl bg-light border-radius-12">
            ...
            <div class="km-heading-7 text-black">{{ m.mcp_noServersYet() }}</div>
            <div class="km-description text-black">{{ m.mcp_addServerToGetStarted() }}</div>
            <div class="cluster mt-lg" data-justify="center">
              <km-btn v-if="canCreate" data-test="new-btn" :label="m.common_addMcpServer()" @click="showNewDialog = true" />
            </div>
          </div>
        </div>
      </template>
    </div>
    <mcp-new-server :show-new-dialog="showNewDialog" @cancel="showNewDialog = false" />
  </div>
</template>
