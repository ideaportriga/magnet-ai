<template>
  ...
  >
    ...
    <kg-dialog-section
      :title="m.retrieval_contextExpansion()"
      :description="m.retrieval_contextExpansionDesc()"
      icon="graph"
      tone="context"
    >
      <template #title-badge>
        <km-badge tone="warning" :label="m.common_comingSoon()" class="text-weight-medium mt-xs" />
      </template>
      <template #header-actions>
        <kg-section-control v-model="localTool.contextControl" :disabled="true" />
      </template>

      <div class="stack section-fields-disabled" data-gap="lg">
        <kg-field-row :cols="2">
          <kg-toggle-field
            :model-value="false"
            :title="m.retrieval_includeReferencedChunks()"
            :description="m.retrieval_includeReferencedChunksDesc()"
            disabled
          />
          <kg-toggle-field
            :model-value="false"
            :title="m.retrieval_includeAdjacentChunks()"
            :description="m.retrieval_includeAdjacentChunksDesc()"
            disabled
          />
        </kg-field-row>
      </div>
