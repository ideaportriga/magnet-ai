...
q-dialog(:model-value='modal', @hide='$emit("update:modal", false)')
  .column.q-pa-md(style="width: 692px; max-width: 800px;  border-radius: 8px; min-height: 668px").bg-background
      .col-auto
        //- Search
        //- icon-before="search"
        km-input.full-width(
      ...
      .col.q-mt-md
        .row.q-gap-12
          template(v-for="icon in pageIcons")
            .col-auto(@click="setIcon(icon)").bg-white.rounded-borders
              .icon-card.q-pa-md
                  div
                    .center-flex-x
                      q-icon(:name="`${icon.name}`" size="26px" color="text-grey").icon-display
      ...
      .q-mt-sm.row.items-center
        .col
          //- div {{pagination}}
        .col-auto
          q-btn(icon="fas fa-angle-left" flat color="grey-7" size="md" @click="page -= 1" :disable="page <= 1").q-px-sm
        .col-auto.q-px-sm
          .km-description {{`${page} of ${pagination.maxPages}`}}
        .col-auto
          q-btn(icon="fas fa-angle-right" flat color="grey-7" size="md" @click="page += 1" :disable="page >= pagination.maxPages").q-px-sm
</template>
...
export default {
  ...
  computed: {
    ...
    displayIcons() {
      ...
      // search
      if (this.search) {
        display = display?.filter(opt => {
          let searchString = Object.entries(opt).filter(([key,val]) => ['name'].includes(key) && val ).map(([,val]) => `${val}`.toLowerCase().replace(/\s/g, '')).join('') ?? ''
