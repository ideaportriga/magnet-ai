...
class PgVectorMetadataFilterBuilder:
    """Builds PostgreSQL JSON query conditions for metadata filtering in pgvector store."""
    ...
    def _format_value(self, value: Any) -> str:
        """Format a value for use in PostgreSQL JSON queries."""
    ...
    def _parse_node(
        ...
        """
        Recursively parses a node in the filter object to build a PostgreSQL condition string.
        ...
        field = operator
        expression = value
        json_path = self._get_json_path(field, field_mapping)
        ...
        if op == "$exists":
            ...
            # For PostgreSQL, check if the JSON key exists
        ...
        try:
            ...
            # For numeric comparisons, cast to numeric if the value is a number
            if isinstance(val, (int, float)):
                return (
                    f"(metadata{json_path})::numeric {sql_operator} {formatted_value}"
                )
            else:
                return f"metadata{json_path} {sql_operator} {formatted_value}"
    ...
    def build(self, collection_config: dict, filter: Optional[FilterObject]) -> str:
        """
        Builds a PostgreSQL WHERE clause condition from a filter object.

        Args:
            collection_config: Configuration containing the metadata field mappings.
            filter: The filter object to build the condition from.

        Returns:
            A WHERE clause condition string, or an empty string if the filter is empty.
        """
        if not filter:
            return ""
        ...
        try:
            ...
            # The recursive parser adds its own parentheses for complex expressions
            # For simple expressions, we might want to remove outer parentheses
            if (
                full_expression.startswith("(")
                and full_expression.endswith(")")
                and full_expression.count("(") == 1
            ):
