...
async def make_multipart_session(
    filename: str, size: int, content_type: str
) -> Dict[str, Any]:
...
# ---------------------------------------------------------------------------
# StorageService-based alternatives (Phase 5)
# ---------------------------------------------------------------------------


def _get_storage_service():
    """Try to get StorageService from app state. Returns None if unavailable."""
    try:
        from advanced_alchemy.types.file_object import storages

        if storages.is_registered("azure"):
            return storages.get_backend("azure")
    except Exception:
        pass
    return None


async def get_read_url_via_storage(object_key: str, minutes: int = 60) -> str:
    """Get a signed read URL via StorageService (obstore) if available, else Azure SDK."""
    backend = _get_storage_service()
    if backend:
        try:
            url = await backend.sign_async(object_key, expires_in=minutes * 60)
            return url  # type: ignore[return-value]
        except (NotImplementedError, ValueError):
            pass
    # Fallback to Azure SDK
    return get_read_url(object_key, minutes)


async def make_multipart_session_via_storage(
    filename: str, size: int, content_type: str
) -> Dict[str, Any]:
    """Create upload session via StorageService if available, else Azure SDK.

    When using obstore, the client-side upload flow remains the same (PUT to
    signed URL).  The signed URL is generated by obstore instead of Azure SDK.
    """
    backend = _get_storage_service()
    if backend:
        try:
            object_key = _mk_object_key(filename)
            upload_url = await backend.sign_async(
                object_key,
                expires_in=15 * 60,
                for_upload=True,
            )
            ...
            pass
    # Fallback to Azure SDK
    return await make_multipart_session(filename, size, content_type)
