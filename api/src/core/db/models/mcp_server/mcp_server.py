...
class MCPServer(UUIDAuditSimpleBase):
    ...
    __table_args__ = (
        Index(
            "uq_mcp_servers_system_name_per_tenant",
            "tenant_id",
            "system_name",
            unique=True,
        ),
        Index("ix_mcp_servers_tenant_id", "tenant_id"),
        Index("ix_mcp_servers_owner_id", "owner_id"),
        Index("ix_mcp_servers_department_id", "department_id"),
        CheckConstraint(
            "visibility IN ('private', 'department', 'tenant')",
            name="ck_mcp_servers_visibility",
        ),
    )
