...
class RetrievalTool(UUIDAuditEntityBase):
    ...
    __table_args__ = (
        Index(
            "uq_retrieval_tools_system_name_per_tenant",
            "tenant_id",
            "system_name",
            unique=True,
        ),
        Index("ix_retrieval_tools_tenant_id", "tenant_id"),
        Index("ix_retrieval_tools_owner_id", "owner_id"),
        Index("ix_retrieval_tools_department_id", "department_id"),
        CheckConstraint(
            "visibility IN ('private', 'department', 'tenant')",
            name="ck_retrieval_tools_visibility",
        ),
    )
