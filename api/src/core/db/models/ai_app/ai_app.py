...
class AIApp(UUIDAuditSimpleBase):
    ...
    __table_args__ = (
        Index(
            "uq_ai_apps_system_name_per_tenant",
            "tenant_id",
            "system_name",
            unique=True,
        ),
        Index("ix_ai_apps_tenant_id", "tenant_id"),
        Index("ix_ai_apps_owner_id", "owner_id"),
        Index("ix_ai_apps_department_id", "department_id"),
        CheckConstraint(
            "visibility IN ('private', 'department', 'tenant')",
            name="ck_ai_apps_visibility",
        ),
    )
