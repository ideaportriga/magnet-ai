...
def schema_upgrades() -> None:
    ...
    sa.Column('type', sa.String(length=100), nullable=True, comment="Provider type (e.g., 'openai', 'azure', 'anthropic')"),
    sa.Column('connection_config', sa.JSON().with_variant(postgresql.JSONB(astext_type=sa.Text), 'cockroachdb').with_variant(advanced_alchemy.types.json.ORA_JSONB(), 'oracle').with_variant(postgresql.JSONB(astext_type=sa.Text), 'postgresql'), nullable=True, comment='Provider configuration (non-sensitive settings)'),
    sa.Column('secrets_encrypted', EncryptedJsonB(key='my-secret-key-tsmh5r'), nullable=True, comment='Encrypted credentials and sensitive connection data'),
    sa.Column('metadata_info', sa.JSON().with_variant(postgresql.JSONB(astext_type=sa.Text), 'cockroachdb').with_variant(advanced_alchemy.types.json.ORA_JSONB(), 'oracle').with_variant(postgresql.JSONB(astext_type=sa.Text), 'postgresql'), nullable=True, comment='Additional metadata about the provider'),
    ...
    op.execute("UPDATE ai_models SET provider_name = provider WHERE provider_name IS NULL")
...
def schema_downgrades() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(op.f('fk_collections_provider_system_name_providers'), 'collections', type_='foreignkey')
    ...
    op.add_column('ai_models', sa.Column('provider', sa.VARCHAR(length=100), autoincrement=False, nullable=False, comment='AI provider (e.g., azure_open_ai)'))
    op.drop_constraint(op.f('fk_ai_models_provider_system_name_providers'), 'ai_models', type_='foreignkey')
