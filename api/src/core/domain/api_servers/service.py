...
from core.db.models.api_server.api_server import APIServer

from .schemas import ApiServer as ApiServerSchema
from .schemas import ApiServerUpdate


class ApiServersService(service.SQLAlchemyAsyncRepositoryService[APIServer]):
    """API servers service."""

    class Repo(repository.SQLAlchemyAsyncRepository[APIServer]):
        """API servers repository."""

        model_type = APIServer

    repository_type = Repo

    async def update(
        self, data: ApiServerUpdate, item_id: Any, auto_commit: bool = False, **kwargs
    ) -> APIServer:
        """
        Update API server with special logic for secrets_encrypted.
    ...
    async def get_with_secrets(self, item_id: Any) -> ApiServerSchema:
        """
        Get API server with full secrets for internal service operations.
        ...
        return ApiServerSchema.model_validate(obj, from_attributes=True)

    async def get_with_secrets_by_system_name(
        self, system_name: str
    ) -> ApiServerSchema:
        """
        Get API server by system_name with full secrets for internal service operations.
    ...
    async def list_with_secrets(self, *filters) -> list[ApiServerSchema]:
        """
        List API servers with full secrets for internal service operations.
        ...
        return [
            ApiServerSchema.model_validate(obj, from_attributes=True) for obj in results
        ]
