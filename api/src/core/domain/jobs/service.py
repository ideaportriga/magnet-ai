        ...
        *filters: FilterTypes,
        ...
        for json_key, values in jsonb_filters.items():
            if values:
                ...
                value_conditions = []
                for value in values:
                    # Handle different JSON paths based on the key
                    if json_key == "run_configuration_type":
                        # For run_configuration.type, use nested JSON path
                        value_conditions.append(
                            f"definition->'run_configuration'->>'type' = '{value}'"
                        )
                    else:
                        # For other fields, use direct JSON path
                        value_conditions.append(
                            f"definition->>'{json_key}' = '{value}'"
                        )
        ...
        async with self.repository.session as session:
            ...
            # Convert rows to Job instances
            items = []
            for row in rows:
                # Create Job instance from row data
