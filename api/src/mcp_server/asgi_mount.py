...
async def _forward_to_mcp(scope: Scope, receive: Receive, send: Send) -> None:
    """Forward a scope to FastMCP's Starlette app.

    FastMCP's streamable_http_app() registers a Route("/mcp", ...) — NOT a
    Mount.  Starlette's Route matches the exact path "/mcp", so we must ensure
    the scope path is "/mcp" (no trailing slash).  If we passed "/mcp/",
    the Route wouldn't match and Starlette's redirect_slashes=True would fire a
    307 back to "/mcp" using the ASGI scope's local host/scheme, which breaks
    when the app sits behind a reverse proxy (e.g. Cloudflare tunnel).
    """
