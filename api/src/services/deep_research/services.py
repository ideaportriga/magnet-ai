...
async def execute_deep_research(
    ...
    try:
        ...
        if (
            run.status == DeepResearchStatus.COMPLETED
            and config.webhook_tool_server
            and config.webhook_tool_name
        ):
...
async def _call_webhook(run: DeepResearchRun, config: DeepResearchConfig) -> None:
    """Call webhook API tool to notify completion."""
    try:
        if not config.webhook_tool_server or not config.webhook_tool_name:
        ...
        if config.webhook_payload_template:
            ...
            payload = _resolve_webhook_template(run, config.webhook_payload_template)

        print("Calling webhook with payload:")
        print(json.dumps(payload, indent=2))
        api_call = ApiToolCall(
            server=config.webhook_tool_server,
            tool=config.webhook_tool_name,
            input_params=ApiToolCallInputParams(
                requestBody=payload if payload else None
            )
        )

        await call_api_server_tool(api_call)
        logger.info(f"Webhook called for run {run.run_id}")
