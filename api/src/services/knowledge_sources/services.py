...
def _build_metadata_mapping(data: dict, prefix: str = "$", name_prefix: str = "", mapping: dict[str, MetadataAutomapField] | None = None, exclude_fields: list[str] | None = None) -> dict[str, MetadataAutomapField]:
    if mapping is None:
        mapping = {}
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            _build_metadata_mapping(value, full_key, simple_name, mapping, exclude_fields)
        else:
...
async def automap_metadata(collection_id: str, exclude_fields: list[str] | None = None):
    ...
    for document in documents:
        ...
        _build_metadata_mapping(metadata, mapping=mapping, exclude_fields=exclude_fields)
    return mapping
