...
def _build_confluence_storage_body(content: str) -> dict[str, str]:
    # Confluence REST v2 "storage" expects HTML.
    # Our note body is produced as Markdown; convert a safe subset to HTML so Confluence renders it.
...
def _md_inline_to_storage_html(text: str) -> str:
    ...
    # Basic inline formatting (best-effort).
...
def _markdown_to_confluence_storage_html(markdown_text: str) -> str:
    ...
    for raw in lines:
        ...
        if bullet_match:
            ...
            if not in_list:
                parts.append("<ul>")
                in_list = True
            parts.append(f"<li>{_md_inline_to_storage_html(bullet_match.group(1) or '')}</li>")
            continue
    ...
    # Confluence storage requires a single root is not required; return combined HTML.
...
def _build_confluence_v2_create_page_request(
    *,
    ...
    if parent_id:
        # Confluence REST v2 uses camelCase.
...
async def _maybe_publish_confluence_notes(
    context: TurnContext,
    *,
    ...
    space_key = str(confluence.get("space_key") or "").strip()
    if not space_key:
        logger.warning("[teams note-taker] confluence enabled but space_key is empty")
        return
    ...
    content = _build_confluence_markdown(
        meeting=meeting_context,
        participants=participants,
        conversation_date=conversation_date,
        conversation_time=conversation_time,
    ...
    # NOTE: The Atlassian Confluence REST v2 "Create page" expects `spaceId` (not `space_key`)
    # and `body.representation/value`. Many API-server tools will map directly to this schema.
    parent_id = str(confluence.get("parent_id") or "").strip()
    arguments: dict[str, Any] = _build_confluence_v2_create_page_request(
        space_id=space_key,
        title=title,
        content=content,
    ...
    debug_payload = {
        ...
        "content_sha256": _hash_text_sha256(content),
    ...
    try:
        api_call = ApiToolCall(
            ...
            input_params=ApiToolCallInputParams(requestBody=arguments),
        )
        ...
        await _send_expandable_section(
            ...
            content=json.dumps(debug_payload, indent=2, sort_keys=True),
            preserve_newlines=True,
        )
        await _send_expandable_section(
            context,
            title="Confluence error details",
            content=json.dumps(_exception_debug_details(err), indent=2, sort_keys=True),
            preserve_newlines=True,
        )
        await context.send_activity(
    ...
    if getattr(result, "status_code", 0) >= 400:
        await _send_expandable_section(
            ...
            content=json.dumps(
                _api_tool_result_debug_details(result), indent=2, sort_keys=True
            ),
            preserve_newlines=True,
        )
        await context.send_activity(
